    ...
    function(data) {
      ...
      if (datadic.hasOwnProperty("list") && datadic.list.length > 0) {
        ...
        $(datadic.list).each(function() {
          ...
          html +=
            '                                    <p class="subName limit-length"><i class="icon icon-subIcon"></i>' +
            this.fSubname +
            "</p>";
          html +=
            "                                    <P>" +
            Operation["ui_MeterName"] +
            (this.fDevicename ? this.fDevicename : "") +
            "</P>";
          html +=
            "                                    <p>" +
            Operation["ui_EventType"] +
            (this.fMessInfoExplain ? this.fMessInfoExplain : "") +
            "</p>";
          html += "                                </div>";
        ...
        if (timeOutEvent != 0 && longClick == 0) {
          ...
          if ($("#showDiv").is(":visible")) {
            ...
            if (alarmeventlogid) {
              if (isAndroid) {
                android.goToIn(
                  "alarmDetailView.html?alarmeventlogid=" + alarmeventlogid
                );
              } else {
