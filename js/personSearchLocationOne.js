...
//fStateSelectID
var fState = Substation.GetQueryString("fState");
var updateLog = localStorage.getItem("updateLog");
var paramData = JSON.parse(updateLog);
if (fState == 1) {
    markerPerArr.push(paramData);
    lon = paramData.F_Longitude;
    lat = paramData.F_Latitude;
} else if (fState == 2) {
    // (marker.lonc, marker.latc);
    markerCarArr.push(paramData);
    lon = paramData.lonc;
    lat = paramData.latc;
} else {
    // marker.fLongitude, marker.fLatitude
    markersArr.push(paramData);
    lon = paramData.fLongitude;
    lat = paramData.fLatitude;
}
