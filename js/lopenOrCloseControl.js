...
//空调与风机的时候隐藏主控制
if (devicetype == "05003" || devicetype == "05004") {
    // $("#control_btn").hide();
...
function getFirstPage() {
    ...
    $(".infinite-scroll-preoader").html('<div class="preloader"></div>');
    loading = false;
    ...
    Substation.getDataByAjaxNoLoading(
        ...
        function (data) {
            ...
            if (data.hasOwnProperty("list") && data.list.length > 0) {
                ...
                //给按钮名称赋值
                localStorage.setItem("closeLightName", datashowdic["0"]);
                localStorage.setItem("openLightName", datashowdic["1"]);
                localStorage.setItem("closeOtherName", datashowdic["2"]);
                localStorage.setItem("openOtherName", datashowdic["3"]);
                ...
                $(data.list).each(function () {
                    ...
                    if (devicetype == "05002") {
                        //SVG
                        html +=
                            '                                <img src="img/SvgControl.png">';
                    } else if (devicetype == "05004") {
                        //风机
                        html +=
                            '                                <img src="img/FanControl.png">';
                    } else if (devicetype == "05003") {
                        //空调
                        html +=
                            '                                <img src="img/airContol.png" style="margin-bottom:1rem">';
                    } else {
                        html +=
                    ...
                    $(".list-item").click(function (deviceValue) {
                        if ($(".footer_btn").is(":hidden")) {
                            ...
                            window.location.href = "lightingDetail.html?devicetype=" + devicetype + "&fSubid=" + subid;
                        }
                    });
                });
                if (!$(".footer_btn").length || $(".footer_btn").is(":hidden")) {
...
$("#record_btn").click(function () {
    ...
    window.location.href = "deviceControlLog.html?type=" + devicetype;
