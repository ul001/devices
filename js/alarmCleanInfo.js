...
function loadMenu() {
    ...
    Substation.getDataByAjaxNoLoading(
        ...
        function (data) {
            if (
                data.hasOwnProperty("taskAndAlarmEventDetail") &&
                data.taskAndAlarmEventDetail
            ) {
                creatView(data);
            } else {
...
function creatView(dataParam) {
    ...
    if (dataParam) {
        ...
        sb +=
            '                                            <option value="3">待客户停电处理</option>';
        sb +=
            '                                            <option value="4">待线路停电处理</option>';
        sb +=
        ...
        $("#fSolveresult").val(
            Substation.removeUnDefinedStr(taskParam.fSolveresult)
        );
